...
class PPTAttention(Attention):
    ...
    @staticmethod
    def ats_score_assignment_step(attn, v):
        """
        ...
        # print("v_norm", v_norm.shape) # 1, 197
        # 1(B), 6, 197, 197
        # print(attn.shape)
        significance_score = attn[:, :, 0].sum(
            dim=1
        )  # attention weights of CLS token of size [B x T]
        # print("significance_score", significance_score.shape) # 1, 197

        # ATS (Fayyaz et al., 2022) takes the values matrix V into
        # consideration and proposes a more comprehensive token scoring metric
        # score = (A x ||V||) / sum(A x ||V||) 
        ...
        # print("significance_score:", significance_score.shape)
        
        return significance_score

    @staticmethod
    def simple_score_assignment_step(attn, v):
        """
        Token Score Assignment Step.
        :param attn: attention matrix
        :param v: values
    ...
    def forward(
        ...
        # print("attn:", attn.shape) # 1, 6, 197, 197
        # print("v:", v.shape) # 1, 6, 197, 64
        ########### ATS token pruning
        scores = self.ats_score_assignment_step(attn, v) # [B, N-1]
        ...
        # print("_scores:", scores.shape)
        # print("min", torch.min(scores), "max", torch.max(scores)) # 0-2
        ##############################

        # ########### Simple token pruning
        # scores = self.simple_score_assignment_step(attn, v) # [B, N-1]
        # cls_score = 2*torch.ones(scores.shape[0], 1).to(scores.device) # confirm cls tokens are reserved
        # scores = torch.cat([cls_score, scores], dim=-1) # [B, N]
        # # print("min", torch.min(scores), "max", torch.max(scores)) # 0-2
        # ##############################
...
class pp_block_adaptive(Block):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # print("S_op:", S_op.shape, torch.min(S_op), torch.max(S_op), self._pp_info["OP_threshold"])

        if r > 0:    
            # img in batch either go pooling or pruning !!!!
            # pooling: N, reduced_tokens, features
            # pruning: M, reduced_tokens, features
            # N + M = B
            # OP_threshold = 0, all goes pruning
            # OP_threshold = +inf, all goes pooling
            ...
            if pruning_imgs_indices.shape[0] > 0:
                ...
                # print("x_pruning:", x_pruning.shape)
                ...
                # print(pruning_indices, len(pruning_indices))
                # print(pruning_indices.shape)

                # thresholding
                # threshold = 1 / (scores.shape[-1] - 1) * 0.65
                # # print("scores:", scores, scores.shape)
                # pruning_indices = torch.where(scores > threshold)[1]         
                # pruning_indices = torch.unsqueeze(pruning_indices, axis=0)
                # print(pruning_indices.shape)
                # print("")
    
                # pruning_indices = torch.argwhere(scores > threshold).flatten()
                # print(pruning_indices, pruning_indices.shape)

                # print("trace_source", self._pp_info["trace_source"])
                if self._pp_info["trace_source"]:
                ...
                # print("pruning_indices:", pruning_indices)
            ...
            # if x_pooling is not None:
            #     print("x_pooling:", x_pooling.shape)
            # else:
            #     print("x_pooling:", x_pooling)
            # if x_pruning is not None:
            #     print("x_pruning:", x_pruning.shape)
            # else:
            #     print("x_pruning:", x_pruning)
            # print("")
...
def apply_patch(
    ...
    for module in model.modules():
        if isinstance(module, Block):
            # print(blk_idx, module)
            if blk_idx in pp_loc_list: # 3, 6, 9 (total 12))
